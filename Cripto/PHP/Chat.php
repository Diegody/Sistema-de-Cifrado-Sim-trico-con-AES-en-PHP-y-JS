...
class Chat
{
	...
	public function getUserChat($from_user_id, $to_user_id)
	{
		...
		foreach ($userChat as $chat) {
			...
			if ($chat["sender_userid"] == $from_user_id) {
				...
				$conversation .= '<img width="22px" height="22px" src="../JPG/' . $fromUserAvatar . '" alt="" />';
			} else {
				$conversation .= '<li class="replies">';
				$conversation .= '<img width="22px" height="22px" src="../JPG/' . $toUserAvatar . '" alt="" />';
			}
	...
	public function showUserChat($from_user_id, $to_user_id)
	{
		...
		foreach ($userDetails as $user) {
			...
			$userSection = '<img src="../JPG/' . $user['avatar'] . '" alt="" />
				<p>' . $user['username'] . '</p>
	...
	public function Registro($userid, $username, $password, $avatar, $current_sesion, $online)
	{
		$sqlReg = "
		INSERT INTO chat_users (userid, username, password, avatar, current_sesion, online) 
                        VALUES ('$userid', '$username', '$password', '$avatar', '$current_sesion', '$online');";
		mysqli_query($this->dbConnect, $sqlReg);
		$lastInsertId = mysqli_insert_id($this->dbConnect);
		return $lastInsertId;
	}
}
